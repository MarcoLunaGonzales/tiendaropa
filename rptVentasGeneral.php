...
while($datos=mysqli_fetch_array($resp)){	
	...
	if ($numRecibos>0){
		...
		$sqlRecVenta.=" left join  salida_almacenes sa on(r.cod_salida_almacen=sa.cod_salida_almacenes and sa.cod_almacen in (select cod_almacen from almacenes where cod_ciudad=".$global_agencia."))";
		$sqlRecVenta.=" where r.cod_ciudad=".$global_agencia;
		$sqlRecVenta.=" and r.recibo_anulado=0 ";
...
$totMontoTipopago=0;
$sql2="select r.cod_tipopago,sum(r.monto_recibo)  from recibos r where r.cod_ciudad=".$global_agencia." and r.recibo_anulado=0 and resta_ventas_proveedor=0";
...
$totGTO=0;
	...
	while ($datTP = mysqli_fetch_array($respTP)) {
		...
		where s.`cod_tiposalida`=1001 
		and s.salida_anulada=0 and	s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a where a.`cod_ciudad`='$rpt_territorio')";	
		$sqlVTA=$sqlVTA." and s.cod_salida_almacenes not in (select r.cod_salida_almacen  from recibos r where  r.cod_ciudad=".$rpt_territorio." and r.cod_salida_almacen is not null)";
		...
		////////////RECIBOS//////////
		$totalRECTP=0;
		$sqlREC="select r.cod_tipopago,sum(r.monto_recibo)  from recibos r where r.cod_ciudad=".$global_agencia." and r.recibo_anulado=0 and r.resta_ventas_proveedor=0";
