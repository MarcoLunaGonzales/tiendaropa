...
function listaMateriales(f){
	...
	var codProveedor=f.proveedor.value;
	contenedor = document.getElementById('divListaMateriales');
	ajax=nuevoAjax();
	ajax.open("GET", "ajaxListaMaterialesIngreso.php?codTipo="+codTipo+"&codMarca="+codMarca+"&codBarraCod2="+codBarraCod2+"&nombreItem="+nombreItem+"&codProveedor="+codProveedor,true);
...
echo "<tr>";
$sql="select nro_correlativo from preingreso_almacenes where cod_almacen='$global_almacen' order by cod_ingreso_almacen desc";
...
$sql1="select p.cod_proveedor, p.nombre_proveedor from proveedores  p ";

if($cantFuncProv>0){
$sql1.="where p.cod_proveedor in(select cod_proveedor from funcionarios_proveedores where codigo_funcionario=$global_usuario)";
}
$sql1.=" order by p.nombre_proveedor";
$resp1=mysqli_query($enlaceCon,$sql1);
